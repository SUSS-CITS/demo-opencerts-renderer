...
import { SUSSTemplateTranscript } from "../../samples";
import React, { FunctionComponent } from "react";
import { TemplateProps } from "@govtechsg/decentralized-renderer-react-components";
import engLocale from "date-fns/locale/en";

export const TranscriptTemplate: FunctionComponent<TemplateProps<SUSSTemplateTranscript>> = ({ document }) => {
  // Declare what will be needed in the Transcript
  const transcriptData = get(document, "transcript", []);
  const recipientName = get(document, "recipient.name").toUpperCase();
  ...
  const confermentDate = format(get(document, "additionalData.dateOfConferment"), "DD MMMM YYYY", {
    locale: engLocale
  });
      ...
      <div>
        ...
        <div className="container">
              ...
              <table className="w-120 transTextStyle">
                <tbody>
                  <tr>
                    ...
                    <td valign="top" width="45%" style={{ textTransform: "uppercase" }}>
                      : {functions.formatSplitText(recipientName, 30, "\u00A0")}
                    </td>
          ...
          <div className="row topBuffer transTextStyle">
            <div className="col">
              This transcript confirms that the student named above has succesfully completed the listed courses leading
              to the qualification shown below. For an explanation of the terms and symbols used, please see the other
              side of this transcript.
            </div>
          </div>
              ...
              <div className="col">
                <table className="transTextStyle">
                  <tbody>
                    ...
                    creditExempted !== "0.0" && (
                      <tr>
                        <td colSpan="4">Credit Exemption:</td>
                        <td align="center">{creditExempted}</td>
                      </tr>
                    )}
                    ...
                    creditRecog !== "0.0" && (
                      <tr>
                        <td colSpan="4">Credit Recognition taken from approved institutions: </td>
                        <td align="center">{creditRecog}</td>
                      </tr>
                    )}

                    <tr>
                      <td colSpan="4">
                        <b>Total number of credit units counted towards this award: </b>
                      </td>
                      <td align="center">
          ...
          {functions.displayTransRetroLiners(confermentDate)}

          <div className="row">
            <div className="col text-center">--------------- End of Transcript ---------------</div>
          </div>
        </div>
      </div>
    </>
